 ...
 */
    public function es_on_publish() {
        ...
        if ($options['publish']) {
            ...
            //make sure article id's aren't in blacklist
            if ($type === 'post') {
    ...
    public function es_post_query_args($posts, $pages) {
        ...
        $posts ? $post_count = wp_count_posts()->publish : $post_count = 0;
        $pages ? $page_count = wp_count_posts('page')->publish : $page_count = 0;
        ...
        return array($posts_and_pages, $count);
    }
    ...
    public function es_index_posts() {
        ...
        function get_tag_ids($tag_array) {
            $new_tags = [];
        ...
        $blacklist = preg_replace('/\s+/', '', $options['blacklist']);
        $blacklist = explode(',', $blacklist);
        $whitelist = preg_replace('/\s+/', '', $options['whitelist']);
        $whitelist = explode(',', $whitelist);

        $blacklist = get_tag_ids($blacklist);
        $whitelist = get_tag_ids($whitelist);

        //check for including posts/pages
        list($posts_and_pages, $post_count) = $this->es_post_query_args($options['posts'], $options['pages']);
        ...
        update_option('_db-search', $options);
        $msg = $added . " posts added to index '" . $es_index . "' of " . $post_count . " available posts and pages.";
        echo ($msg);
        error_log($msg);
        $opened = fopen($temp, 'w');
