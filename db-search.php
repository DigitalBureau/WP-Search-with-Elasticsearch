 ...
 */
    ...
    public function es_search(){
        ...
        $tax_id     = sanitize_text_field($_POST['data']['tag']);
        $author_id  = sanitize_text_field($_POST['data']['author']);
        $sort_by    = sanitize_text_field($_POST['data']['sortby']);
        $page_index = sanitize_text_field($_POST['data']['pageindex']);
        $within     = sanitize_text_field($_POST['data']['within']);
        $is_custom  = sanitize_text_field($_POST['data']['custom']);
        $range      = sanitize_text_field($_POST['data']['daterange']);
        //validate post data
        foreach ($_POST['data'] as $data_item) {
        ...
        // error_log(print_r($searchparams, 1));
 ...
 */
    ...
    private function populate_tags_list($tag_list, $taxonomy){
        foreach ($tag_list as $tag) {
            ...
            if ((!empty($the_tag)) && (strstr($taxonomy, $tag_id) < 1) && ($the_tag->taxonomy != 'category')) {
                //tag block list by id
                $block_list = [11, 366, 319, 172];

                if (!in_array($tag_id, $block_list)) {
 ...
 */
    ...
    private function populate_tag_buttons($post){
        $button_list = sanitize_text_field($post['data']['tag']);
        $buttons     = explode(",", $button_list);
        //remove whitespace and null elements
        $author_list    = rawurldecode(sanitize_text_field($post['data']['author']));
 ...
 */
    ...
    private function populate_tag_buttons_special_date($from, $to, $tagbuttons_agg, $tagbuttons_date, $is_custom){
        ...
        switch (true) {
            case ($from == 'now-30d/d'):
                $tagbuttons_agg = '<li class="esSearch--searchHeaderFilters_item">Past 30 Days<button class="esSearch--tagDelete" id="' . $from . '">x</button></li>';
                break;
            case ($from == 'now-90d/d'):
                $tagbuttons_agg = '<li class="esSearch--searchHeaderFilters_item">Past 90 Days<button class="esSearch--tagDelete" id="' . $from . '">x</button></li>';
                break;
            //custom year date
            case ($is_custom == 'true' && $to !== 'now/d+1d'):
                $tagbuttons_agg  = '';
                $tagbuttons_date = '<li class="esSearch--searchHeaderFilters_item">' . $from_display . ' to ' . $to_display . '<button class="esSearch--tagDelete" id="' . $from . ' to ' . $to . '">x</button></li>';
                break;
            case ($to !== 'now/d+1d'):
                $tagbuttons_agg = '<li class="esSearch--searchHeaderFilters_item">' . date("Y", strtotime($from)) . '<button class="esSearch--tagDelete" id="' . $from . '">x</button></li>';
                break;
        }
 ...
 */
    ...
    private function populate_date_list($response, $is_custom, $isYear, $from, $results_count){
        ...
        $this_year_count = $response['aggregations']['years']['buckets']['2016']['doc_count'];
        ...
        switch ($from) {
            case 'now-30d/d':
                $dates = '<li><a href="" class="esSearch--searchFilters_dateFilter esSearch--searchFilters_filter" id="now-30d/d">Past 30 Days</a><span class="esSearch--searchFilters_filter_span">(' . $day_30 . ')</span></li>';
                break;
            case 'now-90d/d':
                $dates = '<li><a href="" class="esSearch--searchFilters_dateFilter esSearch--searchFilters_filter" id="now-90d/d">Past 90 Days</a><span class="esSearch--searchFilters_filter_span">(' . $day_90 . ')</span></li>';
                break;
            case $start_of_year && $isYear:
                $dates = '<li><a href="" class="esSearch--searchFilters_dateFilter esSearch--searchFilters_filter" id="' . $from . '">' . $this_year . '</a><span class="esSearch--searchFilters_filter_span">(' . $results_count . ')</span></li>';
                break;
        }
